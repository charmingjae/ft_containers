...
template <bool> class _LIBCPP_TEMPLATE_VIS __vector_base_common {
  protected:
...
_LIBCPP_EXTERN_TEMPLATE(
    class _LIBCPP_EXTERN_TEMPLATE_TYPE_VIS __vector_base_common<true>)

template <class _Tp, class _Allocator>
class __vector_base : protected __vector_base_common<true> {
  public:
    typedef _Allocator allocator_type; // Allocator -> allocator_type
    typedef allocator_traits<allocator_type>
        __alloc_traits; // allocator_traits -> alloc_traits = allocator_type의
                        // type 추론 결과를 alloc_traits로 지정
    typedef typename __alloc_traits::size_type size_type; // size_type

  protected:
    typedef _Tp value_type;        // 템플릿 매개변수 -> 값의 타입
    typedef value_type &reference; // 값의 타입을 레퍼런스로
    typedef const value_type &const_reference; // 값의 타입을 const reference로
    typedef
        typename __alloc_traits::difference_type difference_type; // 포인터의 갭
    typedef typename __alloc_traits::pointer pointer;             // 포인터
    typedef
        typename __alloc_traits::const_pointer const_pointer; // const pointer
    typedef pointer iterator;                                 // iterator
    typedef const_pointer const_iterator;                     // const iterator

    pointer __begin_; // vector 의 begin pointer
    pointer __end_;   // vector의 end pointer
    __compressed_pair<pointer, allocator_type>
        __end_cap_; // capacity pointer -> first = pointer, second = allocator
                    // 타입
    ...
    __alloc() const _NOEXCEPT { // const allocator_type을 반환
        return __end_cap_.second();
    }
    _LIBCPP_INLINE_VISIBILITY
    ...
    const pointer &__end_cap() const _NOEXCEPT {
        return __end_cap_.first();
    } // const capacity end 포인터 반환

    _LIBCPP_INLINE_VISIBILITY
    __vector_base() // vector base 기본 생성자 예외 발생 안함
        _NOEXCEPT_(is_nothrow_default_constructible<allocator_type>::value);
    _LIBCPP_INLINE_VISIBILITY __vector_base(
        const allocator_type &__a); // const reference allocator_type을
                                    // 매개변수로 받아오는 vector_base 생성자
#ifndef _LIBCPP_CXX03_LANG
    _LIBCPP_INLINE_VISIBILITY __vector_base(allocator_type &&__a) _NOEXCEPT;
#endif
    ~__vector_base(); // vector_base 소멸자
    ...
    // vector_base member function ( protected )
    _LIBCPP_INLINE_VISIBILITY
    void clear() _NOEXCEPT {
        __destruct_at_end(__begin_);
    } // __destruct_at_end를 실행시켜 vector 내의 모든 element를 삭제
    _LIBCPP_INLINE_VISIBILITY
    size_type capacity() const _NOEXCEPT { // 총 용량을 반환함
        return static_cast<size_type>(__end_cap() - __begin_);
    }

    _LIBCPP_INLINE_VISIBILITY
    void __destruct_at_end(
        pointer __new_last) _NOEXCEPT; // __destruct_at_end의 원형 (선언)

    _LIBCPP_INLINE_VISIBILITY
    void
    __copy_assign_alloc(const __vector_base &
                            __c) { // __copy_assign_alloc 복사 할당자 allocator?
        __copy_assign_alloc( // 재귀로 실행하는 것 같이 보인다
            __c,
            integral_constant<
                bool, __alloc_traits::propagate_on_container_copy_assignment::
                          value>());
    }

    _LIBCPP_INLINE_VISIBILITY // 이동 allocator?
        void
        __move_assign_alloc(__vector_base &__c) _NOEXCEPT_(
            !__alloc_traits::propagate_on_container_move_assignment::value ||
            is_nothrow_move_assignable<allocator_type>::value) {
        __move_assign_alloc(
            __c,
            integral_constant<
                bool, __alloc_traits::propagate_on_container_move_assignment::
                          value>());
    } // enable_if를 활용해 본 함수를 구현 하는지 궁금하다

  private: // private 접근 제한자
    _LIBCPP_INLINE_VISIBILITY
...
__vector_base<_Tp, _Allocator>::__destruct_at_end( // __destruct_at_end 구현부.
    pointer __new_last) _NOEXCEPT { // pointer 타입의 파라미터를 가져온다 보통
                                    // 파라미터는 begin pointer가 들어온다
    pointer __soon_to_be_end = __end_; // 포인터 assign
    while (__new_last !=
           __soon_to_be_end) // while문을 돌면서 뒤에서부터 element를 destroy
                             // 시킨다. 이 때 allocate 된 공간은 deallocate 되지
                             // 않음
        __alloc_traits::destroy(__alloc(),
                                _VSTD::__to_address(--__soon_to_be_end));
    __end_ = __new_last; // __end_ 포인터 치환
}

template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY
__vector_base<_Tp, _Allocator>::__vector_base() // vector_base 생성자 구현부
    _NOEXCEPT_(is_nothrow_default_constructible<allocator_type>::value)
    : __begin_(nullptr),
      __end_(nullptr), // begin, end, endcap 초기화 -> nullptr
      __end_cap_(nullptr, __default_init_tag()) {}

template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY __vector_base<_Tp, _Allocator>::__vector_base(
    const allocator_type &__a) // allocator_type const reference를 파라미터로
                               // 갖고 오는 vector_base 생성자
    : __begin_(nullptr), __end_(nullptr), __end_cap_(nullptr, __a) {
} // begin, end를 nullptr로, end_cap을 __a로 초기화(?) nullptr로 초기화 해야
  // 하지 않나?
// end_cap은 first와 second로 나누어져있는데, first는 pointer, second는
// allocator_type이다. 따라서 pointer는 nullptr, allocator_type은 __a이기 때문에
// 사실상 nullptr가 아닌가?

#ifndef _LIBCPP_CXX03_LANG // 버전 맞지 않아 스킵
template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY
...
template <class _Tp, class _Allocator>
__vector_base<_Tp, _Allocator>::~__vector_base() { // vector_base 소멸자
    if (__begin_ !=
        nullptr) { // begin이 nullptr가 아니면... 즉 vector가 비어있지 않을 때
        clear(); // clear -> element 삭제
        __alloc_traits::deallocate(
            __alloc(), __begin_, capacity()); // 삭제 후 할당된 공간 deallocate
    }
}
...
template <class _Tp, class _Allocator /* = allocator<_Tp> */> // vector class
class _LIBCPP_TEMPLATE_VIS vector
    : private __vector_base<_Tp, _Allocator> { // vector_base를 private로
                                               // 상속받아 사용
  private:                                     // private 접근 제한자
    typedef __vector_base<_Tp, _Allocator>
        __base; // __vector_base를 __base로 지정
    typedef allocator<_Tp>
        __default_allocator_type; // allocator<_Tp> 인스턴스 타입을
                                  // __default_allocator_type으로 지정

  public:
    typedef vector __self;             // 자기 자신
    typedef _Tp value_type;            // 값의 타입 ( template parameter )
    typedef _Allocator allocator_type; // allocator 타입 ( template parameter )
    typedef typename __base::__alloc_traits
        __alloc_traits; // __base(vector_base)의 alloc_traits
    typedef typename __base::reference reference; // base의 reference
    typedef typename __base::const_reference const_reference; // base의 ...
    typedef typename __base::size_type size_type;             // base의 ...
    typedef typename __base::difference_type difference_type; // base의 ...
    typedef typename __base::pointer pointer;                 // base의 ...
    typedef typename __base::const_pointer const_pointer;     // base의 ...
    typedef __wrap_iter<pointer>
        iterator; // wrap_iter<pointer> 인스턴스 타입을 iterator로 지정
    typedef __wrap_iter<const_pointer>
        const_iterator; // wrap_iter<const_pointer> 인스턴스 타입을
                        // const_iterator로 지정
    typedef _VSTD::reverse_iterator<iterator>
        reverse_iterator; // reverse_iterator<iterator> type 재정의
    typedef _VSTD::reverse_iterator<const_iterator>
        const_reverse_iterator; // reverse_iterator<const_iterator> 타입 재정의
    ...
    _LIBCPP_INLINE_VISIBILITY
    vector &operator=(vector &&__x) _NOEXCEPT_(
    ...
    _LIBCPP_INLINE_VISIBILITY
    allocator_type get_allocator() const _NOEXCEPT { return this->__alloc(); }
    ...
    _LIBCPP_INLINE_VISIBILITY
    value_type *data() _NOEXCEPT { return _VSTD::__to_address(this->__begin_); }
    _LIBCPP_INLINE_VISIBILITY
...
#else
    template <class _Arg>
    _LIBCPP_INLINE_VISIBILITY void __emplace_back(_Arg &&__arg) {
  ...
  private:
    ...
    void __swap_out_circular_buffer(
        __split_buffer<value_type, allocator_type &> &__v);
    pointer __swap_out_circular_buffer(
...
#ifndef _LIBCPP_CXX03_LANG
    template <class _Up>
    _LIBCPP_INLINE_VISIBILITY inline void __push_back_slow_path(_Up &&__x);

    template <class... _Args>
    _LIBCPP_INLINE_VISIBILITY inline void
    ...
    // We call annotatations only for the default Allocator because other
    // allocators may not meet the AddressSanitizer alignment constraints. See
    // the documentation for __sanitizer_annotate_contiguous_container for more
    // details.
...
#else
    _LIBCPP_INLINE_VISIBILITY
    void __annotate_contiguous_container(const void *, const void *,
                                         const void *,
    ...
    struct _ConstructTransaction {
      ...
      private:
        ...
        _ConstructTransaction &
        operator=(_ConstructTransaction const &) = delete;
    };

    template <class... _Args>
    _LIBCPP_INLINE_VISIBILITY void __construct_one_at_end(_Args &&...__args) {
...
template <class _InputIterator,
          class _Alloc = typename std::allocator<
...
template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY vector<_Tp, _Allocator>::vector(vector &&__x)
#if _LIBCPP_STD_VER > 14
    _NOEXCEPT
#else
    _NOEXCEPT_(is_nothrow_move_constructible<allocator_type>::value)
...
template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY vector<_Tp, _Allocator> &
vector<_Tp, _Allocator>::operator=(vector &&__x) _NOEXCEPT_(
    (__noexcept_move_assign_container<_Allocator, __alloc_traits>::value)) {
    __move_assign(
        __x,
        integral_constant<
...
vector<_Tp, _Allocator>::assign(_ForwardIterator __first,
    ...
    size_type __new_size =
        static_cast<size_type>(_VSTD::distance(__first, __last));
...
template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY typename vector<_Tp, _Allocator>::iterator
...
template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY typename vector<_Tp, _Allocator>::reference
...
void vector<_Tp, _Allocator>::shrink_to_fit() _NOEXCEPT {
    if (capacity() > size()) {
        ...
        try {
            ...
            allocator_type &__a = this->__alloc();
            __split_buffer<value_type, allocator_type &> __v(size(), size(),
                                                             __a);
            __swap_out_circular_buffer(__v);
#ifndef _LIBCPP_NO_EXCEPTIONS
        } catch (...) {
...
#endif
{
    ...
    __alloc_traits::construct(__a, _VSTD::__to_address(__v.__end_),
...
vector<_Tp, _Allocator>::erase(const_iterator __first, const_iterator __last) {
    ...
    _LIBCPP_ASSERT(
        ...
        " referring to this vector");
    _LIBCPP_ASSERT(
        __get_const_db()->__find_c_from_i(&__last) == this,
        "vector::erase(iterator,  iterator) called with an iterator not"
        " referring to this vector");
#endif
    ...
    if (__first != __last) {
        this->__destruct_at_end(
            _VSTD::move(__p + (__last - __first), this->__end_, __p));
        this->__invalidate_iterators_past(__p - 1);
    }
    iterator __r = __make_iter(__p);
    return __r;
...
vector<_Tp, _Allocator>::insert(const_iterator __position,
    ...
    if (this->__end_ < this->__end_cap()) {
        ...
        allocator_type &__a = this->__alloc();
        __split_buffer<value_type, allocator_type &> __v(
            __recommend(size() + 1), __p - this->__begin_, __a);
        __v.push_back(__x);
    ...
    if (this->__end_ < this->__end_cap()) {
        if (__p == this->__end_) {
            ...
            __temp_value<value_type, _Allocator> __tmp(
                this->__alloc(), _VSTD::forward<_Args>(__args)...);
            __move_range(__p, this->__end_, __p + 1);
    ...
    if (__n > 0) {
        if (__n <= static_cast<size_type>(this->__end_cap() - this->__end_)) {
            ...
            if (__n > 0) {
                ...
                const_pointer __xr =
                    pointer_traits<const_pointer>::pointer_to(__x);
                if (__p <= __xr && __xr < this->__end_)
                    __xr += __old_n;
...
#endif
{
    ...
    __swap_allocator(
        this->__alloc(), __x.__alloc(),
        integral_constant<
            bool, __alloc_traits::propagate_on_container_swap::value>());
#if _LIBCPP_DEBUG_LEVEL >= 2
    __get_db()->swap(this, &__x);
#endif // _LIBCPP_DEBUG_LEVEL >= 2
}
...
template <class _Tp, class _Allocator>
bool vector<_Tp, _Allocator>::__dereferenceable(
    const const_iterator *__i) const {
    return this->__begin_ <= __i->base() && __i->base() < this->__end_;
}

template <class _Tp, class _Allocator>
bool vector<_Tp, _Allocator>::__decrementable(const const_iterator *__i) const {
    return this->__begin_ < __i->base() && __i->base() <= this->__end_;
}

template <class _Tp, class _Allocator>
bool vector<_Tp, _Allocator>::__addable(const const_iterator *__i,
                                        ptrdiff_t __n) const {
...
template <class _Tp, class _Allocator>
bool vector<_Tp, _Allocator>::__subscriptable(const const_iterator *__i,
                                              ptrdiff_t __n) const {
    const_pointer __p = __i->base() + __n;
    return this->__begin_ <= __p && __p < this->__end_;
}
...
class _LIBCPP_TEMPLATE_VIS vector<bool, _Allocator>
  ...
  private:
    ...
    _LIBCPP_INLINE_VISIBILITY
    const size_type &__cap() const _NOEXCEPT { return __cap_alloc_.first(); }
    _LIBCPP_INLINE_VISIBILITY
    __storage_allocator &__alloc() _NOEXCEPT { return __cap_alloc_.second(); }
    _LIBCPP_INLINE_VISIBILITY
    ...
    static const unsigned __bits_per_word =
        static_cast<unsigned>(sizeof(__storage_type) * CHAR_BIT);
  ...
  public:
    ...
    _LIBCPP_INLINE_VISIBILITY
    vector &operator=(vector &&__v) _NOEXCEPT_(
        (__noexcept_move_assign_container<_Allocator, __alloc_traits>::value));
    ...
    _LIBCPP_INLINE_VISIBILITY
    iterator begin() _NOEXCEPT { return __make_iter(0); }
    _LIBCPP_INLINE_VISIBILITY
    const_iterator begin() const _NOEXCEPT { return __make_iter(0); }
    _LIBCPP_INLINE_VISIBILITY
    iterator end() _NOEXCEPT { return __make_iter(__size_); }
    _LIBCPP_INLINE_VISIBILITY
    const_iterator end() const _NOEXCEPT { return __make_iter(__size_); }
    ...
    _LIBCPP_INLINE_VISIBILITY
    const_iterator cbegin() const _NOEXCEPT { return __make_iter(0); }
    _LIBCPP_INLINE_VISIBILITY
    const_iterator cend() const _NOEXCEPT { return __make_iter(__size_); }
    _LIBCPP_INLINE_VISIBILITY
    const_reverse_iterator crbegin() const _NOEXCEPT { return rbegin(); }
    _LIBCPP_INLINE_VISIBILITY
    const_reverse_iterator crend() const _NOEXCEPT { return rend(); }
...
#if _LIBCPP_STD_VER > 11
    template <class... _Args>
    _LIBCPP_INLINE_VISIBILITY iterator emplace(const_iterator position,
                                               _Args &&...__args) {
  ...
  private:
    ...
    void __copy_assign_alloc(const vector &__v) {
        __copy_assign_alloc(
            __v,
            integral_constant<
                bool, __storage_traits::propagate_on_container_copy_assignment::
                          value>());
    }
    _LIBCPP_INLINE_VISIBILITY
    ...
    void __move_assign(vector &__c, true_type)
        _NOEXCEPT_(is_nothrow_move_assignable<allocator_type>::value);
    _LIBCPP_INLINE_VISIBILITY
    void __move_assign_alloc(vector &__c) _NOEXCEPT_(
        !__storage_traits::propagate_on_container_move_assignment::value ||
        is_nothrow_move_assignable<allocator_type>::value) {
        __move_assign_alloc(
            __c,
            integral_constant<
                bool, __storage_traits::propagate_on_container_move_assignment::
                          value>());
    }
    _LIBCPP_INLINE_VISIBILITY
    ...
    _LIBCPP_INLINE_VISIBILITY
    void __move_assign_alloc(vector &, false_type) _NOEXCEPT {}
...
//  Precondition:  __new_size > capacity()
template <class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY typename vector<bool, _Allocator>::size_type
...
vector<bool, _Allocator>::__construct_at_end(size_type __n, bool __x) {
    ...
    if (__old_size == 0 || ((__old_size - 1) / __bits_per_word) !=
        ...
        if (this->__size_ <= __bits_per_word)
            ...
            this->__begin_[(this->__size_ - 1) / __bits_per_word] =
                __storage_type(0);
    }
...
template <class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY vector<bool, _Allocator>::vector()
    _NOEXCEPT_(is_nothrow_default_constructible<allocator_type>::value)
...
template <class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY vector<bool, _Allocator> &
vector<bool, _Allocator>::operator=(vector &&__v) _NOEXCEPT_(
    (__noexcept_move_assign_container<_Allocator, __alloc_traits>::value)) {
    __move_assign(
        __v,
        integral_constant<
...
template <class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY typename vector<bool, _Allocator>::iterator
...
size_t vector<bool, _Allocator>::__hash_code() const _NOEXCEPT {
    ...
    // do last partial word
    if (__n > 0) {
        const __storage_type __m =
            ~__storage_type(0) >> (__bits_per_word - __n);
        __h ^= *__p & __m;
    }
    return __h;
}
...
template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY void swap(vector<_Tp, _Allocator> &__x,
...
inline _LIBCPP_INLINE_VISIBILITY void erase(vector<_Tp, _Allocator> &__c,
                                            const _Up &__v) {
    __c.erase(_VSTD::remove(__c.begin(), __c.end(), __v), __c.end());
}

template <class _Tp, class _Allocator, class _Predicate>
inline _LIBCPP_INLINE_VISIBILITY void erase_if(vector<_Tp, _Allocator> &__c,
                                               _Predicate __pred) {
